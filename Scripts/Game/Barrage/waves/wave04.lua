...
local W={arena={x=320,y=315,w=280,h=84},minTravel=58,
    stages={"单灯开场", "单灯向左", "阴影出刀 · 左", "单灯向右", "阴影出刀 · 右", "拉上幕布", "单灯向左", "光中出刀 · 左", "单灯向右", "光中出刀 · 右", "扯下幕布"}}
...
function W.enter(m)
    ...
    elseif s==2 or s==4 or s==7 or s==9 then
...
function W.lighting(m)
    local s=m.stage
    if s==2 or s==4 or s==7 or s==9 then
...
function W.update(m)
    local s=m.stage
    if s==1 or s==6 then
        if s==6 and m.caption[2]=="Wave04.Curtain" and m:dialogueDone() then
            m.caption={"Nap","Wave04.Nap"}
        elseif m.phaseTime>(s==6 and Curtain.ENTER_TIME or 1) and m:dialogueDone() then m:next() end
    elseif s==11 then
